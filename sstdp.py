...
def T(T_mean: float, total_out_spike_count: int, y: np.ndarray) -> list:
    ...
    for out_neuron_i in range(Layer_sizes[Layer_out_i]):
        ...
        if out_neuron_i == y:
            if total_out_spike_count == 0:
                expected_firing_times.append(Delta)
            else:
                expected_firing_times.append(min(spike_time if spike_time != 0 else T_max, T_mean - ((total_out_spike_count - 1) / total_out_spike_count) * Delta))
        else:
            if total_out_spike_count == 0:
                expected_firing_times.append(T_max)
            else:
